	...
	public void createPartControl(Composite parent) {
		...
		ISelectionListener listener = new ISelectionListener() {
			...
			public void selectionChanged(IWorkbenchPart part, ISelection selection) {
				...
				if(o instanceof FolderItem){
					Collection<Message> messages = ((FolderItem) o).getMessages();
		...
		tableComposite.addControlListener(new ControlAdapter() {
			public void controlResized(ControlEvent e) {
				tableComposite.setSize(tableComposite.getSize().x, parent
						.getParent().getSize().y - 40);
				TableColumn[] colmns = tableViewer.getTable().getColumns();
				for (int i = 2; i < colmns.length; i++) {
					colmns[i].setWidth((tableComposite.getSize().x
							- colmns[0].getWidth() - colmns[1].getWidth() - 30)
							/ (colmns.length - 2));
				}
			}
		});
